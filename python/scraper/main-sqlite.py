...
# ニュース記事を、全てSQLiteデーテベースに投げる
def save_articles_sqlite(articles):
    with closing(sqlite3.connect('ヤフー記事.db')) as conn:
        c = conn.cursor()
        create_table = 'CREATE TABLE IF NOT EXISTS articles (url TEXT, title TEXT, contents TEXT)'
        c.execute(create_table)
        c.executemany('INSERT INTO articles (url, title, contents) values (?,?,?)', articles)
        conn.commit()
